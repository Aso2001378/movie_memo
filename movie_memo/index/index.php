...
echo 'ユーザーID：<input type="text" name="user_id" value="'.$_SESSION['user']['id'].'" readonly>','<br>';
echo '</div>';
}
...
if(isset($_POST['check'])){
    ...
    foreach($check as $name){
        //チェックされている
        $sql=$pdo->prepare('delete from text where user_id = "'.$_SESSION['user']['id'].'" and check_id = "'.$name.'"');
...
//ユーザーIDによって違う内容を降順に表示
echo '<hr>';
if(isset($_SESSION['user']['id'])){
$sql=$pdo->prepare('select * from text where user_id = "'.$_SESSION['user']['id'].'" order by check_id desc');
$sql->execute();
foreach($sql as $row){
