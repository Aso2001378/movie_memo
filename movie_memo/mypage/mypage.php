...
//メールアドレス重複チェック
    ...
    foreach($sql as $row){
        ...
        if(empty($mail_result) && !empty($_POST['mail'])){
            $sql = $pdo->prepare('update user set mail=? where user_id =  "'.$_SESSION['user']['id'].'"');
            $sql -> bindvalue(1,$_POST['mail']);
            $sql -> execute();
                $result = '<p>メールアドレスを変更しました</p>';
                echo '<hr>';
                echo $result;
        }
    ...
    if(empty($mail_result) && !empty($_POST['mail'])){
    echo '<div class="change_info">';
    echo '変更後のメールアドレス：'.$_POST['mail'],'<br>';
    echo '</div>';
    echo '<hr>';
    }
