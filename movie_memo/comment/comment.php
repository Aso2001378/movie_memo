...
echo '<input id="user_name" type="text" class="user" name="user_name"  value="'.$_SESSION['user']['user'].'" readonly><br>';//SESSION['user']に保持されているユーザー名を表示
}else{
...
echo '<hr>';
    ...
    $sql=$pdo->prepare("select * from comment order by post_at desc");
    $sql->execute();
        foreach($sql as $row){
